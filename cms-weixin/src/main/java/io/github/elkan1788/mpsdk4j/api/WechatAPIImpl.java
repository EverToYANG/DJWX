...
public class WechatAPIImpl implements WechatAPI {
	...
	@Override
	public boolean delTemplate(String tmlId) {
		   String url = mergeCgiBinUrl(del_template + getAccessToken());
	        ApiResult ar = null;
	        String data = "{\"template_id\":\"" + tmlId + "\"}";
	        for (int i = 0; i < RETRY_COUNT; i++) {
	            ar = ApiResult.create(HttpTool.post(url, data));
	            if (ar.isSuccess()) {
	                return true;
	            ...
	            log.errorf("delete mp[%s] template  failed. There try %d items.",
	                       mpAct.getMpId(),
	                       i);
	        }
