...
import com.gsccs.cms.bass.utils.XMLUtil;
import com.gsccs.cms.core.dao.TempletLinkMapper;
 ...
 */
@Service
public class TempletLinkServiceImpl implements TempletLinkService {
	 ...
	 * @return
	 */
	public boolean hasPagemark(String templet, String type, boolean isClass,
			String pagemark) {
		TempletLinkExample example = new TempletLinkExample();
		Criteria criteria = example.createCriteria();
	...
	public void proSearchParam(TempletLink templetLink, Criteria criteria) {
		if (templetLink != null) {
			if (templetLink.getTemplet() != null
					&& templetLink.getTemplet().trim().length() > 0) {
			...
			if (templetLink.getType() != null
					&& templetLink.getType().trim().length() > 0) {
	...
	public void createXML(Templet templet, HttpServletRequest request) {
		if (templet != null) {
			...
			if (list != null && list.size() > 0) {
				...
				linksElement.addAttribute("xmlns:xs",
				...
				for (int i = 0; i < list.size(); i++) {
					...
					if (templetLink.getTemplet() != null) {
						// 添加link子元素 templet
						Element templetElement = linkElement
								.addElement("templet");
				...
				// 生成xml文件
				XMLUtil.writeFile(document, request.getRealPath("/")
						+ "/templet/" + templet.getId() + "/links.xml");
			}
		}
	}
	 ...
	 */
	public void importLinks(Templet templet, HttpServletRequest request)
			throws DocumentException {
		if (templet != null) {
			// 判断links.xml文件是否存在
			File file = new File(request.getRealPath("/") + "/templet/"
					+ templet.getId() + "/links.xml");
			if (file.exists()) {
				...
				for (Iterator iter = root.elementIterator(); iter.hasNext();) {
					...
					for (Iterator iterInner = element.elementIterator(); iterInner
						...
						if (elementInner.getName().equals("id")) {
							...
							try {
								templetLink.setOrdernum(Integer
										.parseInt(elementInner.getText()));
							} catch (Exception e) {
							}
						}
