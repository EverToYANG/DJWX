...
<div class="widget-box collapsible">
							...
							<form class="form-search" action="msgSendlist.do">
								收信人
								
								<input name="tomembername" size="10" type="text" maxlength="50"
									class="input-medium search-query" value="${param.tomembername }"  />
								标题
								
								<input name="title" size="10" type="text" maxlength="50"
									class="input-medium search-query" value="${param.title }"   />
								内容
								
								<input name="content" size="10" type="text" maxlength="500"
									class="input-medium search-query" value="${param.content }"   />
								已读:
								<select name="isread" style="width:70px">
									<option value="" >全部
									<option value="0"  ${"0"==param.isread?"selected":"" }>否
									<option value="1"  ${"1"==param.isread?"selected":"" }>是
								</select>
								系统站内信:
								<select name="issys" style="width:70px">
									<option value="" >全部
...
<th>
	<fs:order colName="时间" col="addime"/>
							...
							<c:if test='${bean.tomembername!=null && ""!=bean.tomembername}'>
							${bean.tomembername }
							</c:if>
							<c:if test='${bean.tomembername==null || ""==bean.tomembername}'>
	...
	<TD   >
		<a href="msgInfo.do?id=${bean.id }">${bean.title }</a>
	</TD>
	<TD   >
		${"1" == bean.isread ?"是":"否" }
	</TD>
	...
	<TD  >
		${bean.addtimeStr }
							...
							<div id="errorDiv" class="alert alert-error hide">
								...
								<div class="navbar">	
								<div class="navbar-inner">
									<button  onclick="del('msgSendlist.do')" class="btn btn-danger"  >删 除</button>
								</div>
								</div>
						</div>
      </div><!--/row-->
