			...
			<div class="widget-box collapsible">
				...
				<div class="collapse in" id="collapseOne">
					<div class="widget-content">
						<form class="form-search" action="statInfoUpdate.do">
							<input type="hidden" id="pageFuncId" name="pageFuncId" value="${param.pageFuncId }" /> <input type="hidden" name="export" id="export" value="" /> 更新周期: <label><input type="radio" name="statType" value="year" ${(statType==null || statType=="" || statType=="year")?"checked":"" } />年</label> <label><input type="radio" name="statType" value="month" ${(statType=="month")?"checked":"" } />月</label> <label><input type="radio" name="statType" value="day" ${(statType=="day")?"checked":"" } />日</label> <label><input type="radio" name="statType" value="week" ${(statType=="week")?"checked":"" } />星期</label> 时间范围： <input name="infostarttime" id="starttime" class="input-medium search-query Wdate" type="text"
								size="24" value="${info.infostarttimeStr }" onClick="WdatePicker({skin:'default',dateFmt:'yyyy-MM-dd HH:mm:ss'})" /> 至 <input name="infoendtime" id="endtime" class="input-medium search-query Wdate" type="text" size="24" value="${info.infoendtimeStr }" onClick="WdatePicker({skin:'default',dateFmt:'yyyy-MM-dd HH:mm:ss'})" /> 每页显示条数： <select name="pageSize" id="pageSize" style="width: 70px">
