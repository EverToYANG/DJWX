		...
		function infoSignComplete(data){
			if(data.indexOf("操作成功")>-1){
				...
				$("#ip"+id).html("<%=request.getRemoteAddr()%>
					...
					if (GetCookie("cms_articlesignLoginName") != null) {
						$("#loginname").val(GetCookie("cms_articlesignLoginName"));
					}
					if (GetCookie("cms_articlesignPwd") != null) {
						$("#pwd").val(GetCookie("cms_articlesignPwd"));
					}
				</script>
			</c:if>
