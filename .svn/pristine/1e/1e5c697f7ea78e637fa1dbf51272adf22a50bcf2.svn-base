...
import com.gsccs.cms.core.dao.LinkMapper;
	...
	public void proSearchParam(Link link,Criteria criteria){
		if (link!=null ) {
			...
			if (link.getClassName()!=null && link.getClassName().trim().length()>0) {
				criteria.andSql(" (parid in (select id from cms_link where name like '%"+link.getClassName()+"%')) ");;
			}
			...
			if (link.getClassPagemarks()!=null && link.getClassPagemarks().trim().length()>0) {
				criteria.andSql(" (parid in (select id from cms_link where pagemark in ('"+link.getClassPagemarks().replaceAll(",", "','")+"'))) ");
			}
		}
	}
	/**
