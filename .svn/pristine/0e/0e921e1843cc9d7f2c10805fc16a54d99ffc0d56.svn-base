...
import com.gsccs.cms.core.dao.VisitMapper;
import com.gsccs.cms.core.model.Visit;
import com.gsccs.cms.core.model.VisitExample;
import com.gsccs.cms.core.model.VisitExample.Criteria;
...
public class VisitServiceImpl implements VisitService {
	...
	public void proSearchParam(Visit visit, Criteria criteria) {
		if (visit != null) {
			if (visit.getChannelname() != null
					&& visit.getChannelname().trim().length() > 0) {
				criteria.andSql(" c.name like '%"
						+ visit.getChannelname().trim() + "%'");
			}
			if (visit.getSitename() != null
					&& visit.getSitename().trim().length() > 0) {
				criteria.andSql(" s.name like '%" + visit.getSitename().trim()
						+ "%'");
			}
			if (visit.getInfoname() != null
					&& visit.getInfoname().trim().length() > 0) {
				criteria.andSql(" i.title like '%" + visit.getInfoname().trim()
						+ "%'");
			}
			if (StringUtils.isNotEmpty(visit.getChannelid())) {
				criteria.andChannelidEqualTo(visit.getChannelid());
			}
			if (StringUtils.isNotEmpty(visit.getSiteid())) {
				criteria.andVisitSiteidEqualTo(visit.getSiteid());
			}
			if (visit.getStarttime() != null) {
				criteria.andVisitAddtimeGreaterThanOrEqualTo(visit
						.getStarttime());
			}
