		...
		<div class="row-fluid">
			<div class="widget-box collapsible">
				...
				<div class="collapse in" id="collapseOne">
					<div class="widget-content">
						<form class="form-search" action="statCommentUpdate.do">
							<input type="hidden" id="pageFuncId" name="pageFuncId" value="${param.pageFuncId }" /> <input type="hidden" name="export" id="export" value="" /> 更新周期: <label><input type="radio" name="statType" value="year" ${(statType==null || statType=="" || statType=="year")?"checked":"" } />年</label> <label><input type="radio" name="statType" value="month" ${(statType=="month")?"checked":"" } />月</label> <label><input type="radio" name="statType" value="day" ${(statType=="day")?"checked":"" } />日</label> <label><input type="radio" name="statType" value="week" ${(statType=="week")?"checked":"" } />星期</label> 审核状态: <select name="commentstate" style="width: 80px">
								<option value="">全部
								<option value="0" ${"0"==comment.commentstate?"selected":"" }>未审核
								<option value="1" ${"1"==comment.commentstate?"selected":"" }>已审核
							</select> 对象: <select name="objtype" style="width: 80px">
								<option value="">全部
									<c:forEach var="bean" items="${objtypes}" varStatus="status">
										<option value="${bean.key }" ${comment.objtype==bean.key ?"selected":"" }>${bean.value }
									</c:forEach>
							</select> 时间范围： <input name="starttime" id="starttime" class="input-medium search-query Wdate" type="text" size="24" value="${comment.starttimeStr }" onClick="WdatePicker({skin:'default',dateFmt:'yyyy-MM-dd HH:mm:ss'})" /> 至 <input name="endtime" id="endtime" class="input-medium search-query Wdate" type="text" size="24" value="${comment.endtimeStr }" onClick="WdatePicker({skin:'default',dateFmt:'yyyy-MM-dd HH:mm:ss'})" /> 每页显示条数： <select name="pageSize" id="pageSize" style="width: 70px">
							...
							<button type="button" class="btn" onclick="$('#export').val('1');this.form.submit();">
								<i class="icon-download-alt"></i> 导 出
							</button>
						</form>
					</div>
				</div>
			...
			<div class="widget-box">
				...
				<div class="widget-content nopadding">
					...
					<div>
						<div id="jqChart" style="width: 99%; height: 300px;"></div>
					</div>
